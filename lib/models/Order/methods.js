        ...
        if (Roles.isManager(this.userId)) {
            ...
            order.chefs.forEach((item, i) => {
                ...
                if (location.notes)
                    chefsInfo += `\n${location.notes}`
                if (location.latlong)
                    chefsInfo += `\n[Waze](http://waze.to?ll=${location.latlong.replace(/\s/g, "")}&navigate=yes)`
                chefsInfo += `\n\n`
            })


            // Compile finish
            let finish = `**${order.event.location}**`
            if (order.driver.finishDirections)
                finish += `\n${order.driver.finishDirections}`
            if (order.driver.finishLatlong)
                finish += `\n[Waze](http://waze.to?ll=${order.driver.finishLatlong.replace(/\s/g, "")}&navigate=yes)`

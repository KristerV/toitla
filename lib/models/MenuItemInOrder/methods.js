...
Meteor.methods({
    'menuitemInOrder--refreshOrder': function(orderId) {
        ...
        if (!order.submitted || Roles.userIsInRole(this.userId, 'manager')) {
            if(Meteor.isServer) {
                var manager = new MenuItemsInOrderManager(orderId, true)
                manager.refreshOrder()
            }
        } else {
    ...
    'menuitemInOrder--switchItem': function(menuitemId) {
        ...
        if (!order.submitted || Roles.userIsInRole(this.userId, 'manager')) {
            ...
            if(Meteor.isServer) {
                var manager = new MenuItemsInOrderManager(order._id, true)
                manager.switchItem(menuitemId)
            }
        } else {
            throw new Meteor.Error("Not authorized to update MenuItems.")
        }

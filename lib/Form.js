...
Meteor.methods({
    ...
    submitInitialForm: function(docId, calculatedPrice) {
        ...
        var emailText = "Inimesi: " + (order.details.peopleCount || "-") +
        "\r\nTüüp: " + (order.details.serviceType || "-") +
        "\r\nKuupäev: " + moment(order.details.fromDate).format("D. MMM") +
        "\r\nNimi: " + (order.contact.name || "-") +
        "\r\nEmail: " + (order.contact.email || "-") +
        "\r\nNumber: " + (order.contact.number || "-") +
        "\r\nOrganisatsioon: " + (order.contact.organization || "-") +
        "\r\nHind: " + (order.details.calculatedPrice || "-") + "€" +
        "\r\nÜritus: " + (order.details.eventDescription || "-") +
        "\r\nAllergiad: " + (order.details.allergies || "-") +
        "\r\nLisainfo: " + (order.details.clientNotes || "-");
          ...
          Meteor.http.post('https://mandrillapp.com/api/1.0/messages/send.json',
      			{
      				"data": {
      					...
      					"message": {
      						"from_email": "teavitus@toitla.com",
      						"subject": "Uus tellimus toitlas",
      						"text": emailText,
      						"to": [
      							{
      								"email": 'conv.1nho427pw5qh36@fleep.io',
      								"type": "to"
      							}
      						],
      					}

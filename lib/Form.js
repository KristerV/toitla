...
Form = {
    ...
    handleTextFieldChange: function(e, callback) {
        var value = parseInt(e.target.value) || e.target.value
    ...
    checkRequiredFields: function(checkField, reqFields) {
        var errors = {}
        errors[checkField] = {}
        for (var field in this[checkField]) {
            if (!this[checkField].hasOwnProperty(field) || !this[checkField][field]) {
                errors[checkField][field] = T("global", "please_fill")
            }
        }
        return errors
    }

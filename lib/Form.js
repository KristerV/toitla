...
Form = {
    ...
    updatePhase: function(phase) {
        Meteor.call('updatePhase', this._id, phase)
    },
...
Meteor.methods({
    FormUpdateField: function(collectionName, docId, field, value) {
        ...
        if (doc.isSubmitted() && !Roles.userIsInRole(this.userId, 'manager'))
    ...
    updatePhase: function(orderId, phase) {
        check(orderId, String);
        check(phase, String);
        var order = Orders.findOne(orderId)
        if (!Roles.userIsInRole(this.userId, 'manager') && order.isSubmitted())
